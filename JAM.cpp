...
bool solve1(int A, int B, int G1, int G2) {
	...
	if (low[G2] <= num[G1]) return true; // Đảm bảo g1 và g2 không phải là cầu
	if (thuoc(A,G2) == thuoc(B,G2)) return true; // G1 không có ảnh hưởng đến u v
	return false;
}
